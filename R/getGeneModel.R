...
getwd()
setwd("/work/matheus")
gff <- read.table("tmp/data/refGene.txt.gff", sep = "\t", stringsAsFactors=F)
...
colnames(aux) <- c('gene_id', "transcript_id")
head(aux)
gff <- cbind(gff, aux)
head(gff)
...
gff.mrg <- reduce(gff.gr.spt) # gff merged/reduced

gff.mrg

pl1 <- autoplot(gff.mrg[['A1BG']])

pl2 <- autoplot(gff.gr.spt[['A1BG']])
...
gene.model$name <- gene.model$gene
